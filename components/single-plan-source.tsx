...
export function SinglePlanSource({ classes, courses, classId, courseId, category, week, onClassChange, onCourseChange, onCategoryChange, onCourseWeekChange }: {
  ...
  return <div className="single-source-grid">
    ...
    <Label className="field"><span>Lesson category<b className="required-mark"> *</b></span><Input value={category} onChange={(event) => onCategoryChange(event.target.value)} placeholder="e.g. Regular, Midterm, Skills Review" maxLength={200} required /></Label>
    ...
    {selected && <Label className="field"><span>Course week<b className="required-mark"> *</b></span><select className="native-select" value={selected.weeks.some((item) => item.week === week) ? week : ""} onChange={(event) => onCourseWeekChange(Number(event.target.value))} required><option value="" disabled>Choose a week</option>{selected.weeks.map((item) => <option key={item.week} value={item.week}>Week {item.week}: {item.topic}</option>)}</select></Label>}
  </div>;
}
