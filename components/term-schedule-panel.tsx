...
export function TermSchedulePanel({ classes, courses, settings, signedIn, onGenerate, onSignIn, onManageClasses, onManageCourse }: {
  ...
  function selectClass(id: string) { setClassId(id); setOverviewId(""); setMeetingDays(null); setDuration(null); setSchoolYear(""); setContentSource(null); setFormError(null); }
  function clear() { setCategory("Midterm"); setTermStart(""); setNumberOfWeeks(8); setStartingWeek(1); setMeetingDays(null); setDuration(null); setChapter(""); setUnit(""); setContentSource(null); setOverviewId(""); setGenerationMode("template"); setFormError(null); }
  ...
  return <div className="content term-content"><Card className="term-card"><div className="term-card-heading"><span className="term-card-icon"><CalendarRange size={21} /></span><div><h2>Term schedule generator</h2><p>Create one editable lesson draft for each scheduled class meeting.</p></div><span className="template-status"><Sparkles size={14} /> {generationMode === "ai" ? "AI assisted" : "Smart template"}</span></div>
    <form onSubmit={generate} onChange={() => setFormError(null)} className="term-form"><div className="term-form-grid top-row"><Label className="field"><span>Class<b className="required-mark"> *</b></span><select className="native-select" value={selectedClass?.id || ""} onChange={(event) => selectClass(event.target.value)} disabled={!classes.length}><option value="" disabled>Select a class</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.grade}</option>)}</select></Label><Label className="field"><span>Lesson category<b className="required-mark"> *</b></span><select className="native-select" value={category} onChange={(event) => setCategory(event.target.value)}><option>Midterm</option><option>Final</option><option>Weekly</option><option>Regular</option><option>Review</option></select><small>Saved with every lesson in the batch and shown in print.</small></Label><TermField label="School year" value={effectiveYear} onChange={setSchoolYear} /></div>
      <div className="term-form-grid schedule-row"><TermField label="Term start date" type="date" value={termStart} onChange={setTermStart} required /><TermField label="Number of weeks" type="number" min={1} max={24} value={numberOfWeeks} onChange={(value) => setNumberOfWeeks(Number(value))} /><TermField label="Starting week no." type="number" min={1} max={52} value={startingWeek} onChange={(value) => setStartingWeek(Number(value))} /><TermField label="Duration" type="number" min={10} max={240} value={effectiveDuration} onChange={(value) => setDuration(Number(value))} /><GenerationModeSelect value={generationMode} disabled={busy} onChange={(value) => { setGenerationMode(value); setFormError(null); }} /></div>
