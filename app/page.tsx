...
export default function Home() {
  ...
  function reset() { setPlan(null); localStorage.removeItem(workingDraftKey); setDetails({ ...starter, ...settingsToDetails(settings) }); setSingleClassId(""); setSingleCourseId(""); setSingleCategory("Regular"); setTab("goals"); setView("builder"); setBuilderError(null); setPreviewError(null); }
  ...
  return <div className="app-shell">
    ...
    <main className="main-area"><header className="topbar"><div className="mobile-brand">lesson<em>leaf</em></div><span className="breadcrumb">Workspace <b>/</b> {view === "builder" || view === "term" ? "Generate lesson plan" : view === "classes" ? "Classes" : view === "course" ? "Course" : view === "settings" ? "Settings" : view === "guide" ? "Guide" : "My lesson plans"}</span><div className="top-actions"><Badge className="mode-pill"><i /> Template &amp; AI drafting</Badge>{session ? <Button type="button" className="account-button" onClick={logout} title="Sign out">{session.user?.email?.slice(0, 1).toUpperCase() || "✓"}<span>Sign out</span></Button> : <Button type="button" className="sign-in-button" onClick={() => setAuthOpen(true)}>Sign in</Button>}</div></header>
      ...
      : view === "settings" ? <SettingsPanel key={`${session?.user.id ?? "guest"}:${JSON.stringify(settings)}`} settings={settings} session={session} recovery={recovery} onSave={saveSettings} onPasswordChanged={() => setRecovery(false)} onSignIn={() => { setAuthMode("sign-in"); setAuthError(null); setAuthOpen(true); }} />
